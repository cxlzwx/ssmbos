		...
		<div id="loginBlock" class="login"
			...
			<div class="loginForm">
				<form id="loginform" name="loginform" method="post" class="niceform"
					action="${pageContext.request.contextPath}/page_common_index">
					<div id="idInputLine" class="loginFormIpt showPlaceholder"
						style="margin-top: 5px;">
						<input id="loginform:idInput" type="text" name="username"
							class="loginFormTdIpt" maxlength="50" onblur="validateUsername(this.value)"/>
						<label for="idInput" class="placeholder" id="idPlaceholder">帐号：</label>
					</div>
					<span id="validateUsernameMSG"></span>
					...
					<div class="loginFormIpt loginFormIptWiotTh"
						...
						<a href="javascript:document.loginform.submit()"  id="loginform:j_id19" name="loginform:j_id19">
						<span
...
<script type="text/javascript">
    function validateUsername(data){
       $.get(
			"${pageContext.request.contextPath }/validateUsername?username="+data,
			function(result) {
//                alert(result.validateUsernameMSG);
				if(result.validateUsernameMSG == "该用户不能登录"){
                    $("#validateUsernameMSG").css("color","red");
				}else{
                    $("#validateUsernameMSG").css("color","green");
				}
				$("#validateUsernameMSG").html(result.validateUsernameMSG);
            });
